...
export class WebsocketLogger {
  ...
  reset = () => {
    ...
    this.logPath = path.join(LOG_DIR, `${dtStr}.log`);
...
const loggerCache = new Map<string, WebsocketLogger>();
export function getMakeWebsocketLogger(callSid: string) {
  if (!callSid) return new WebsocketLogger("misc");

  let log = loggerCache.get(callSid);
  if (log) return log;

  log = new WebsocketLogger(callSid);
