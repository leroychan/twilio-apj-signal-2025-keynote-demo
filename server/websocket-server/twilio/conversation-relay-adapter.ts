...
export class ConversationRelayAdapter {
  ...
  // ========================================
  /**
   * Registers a listener for incoming websocket messages. All other listeners call this.
   */
  ...
  /**
   * EXPERIMENTAL
   * Registers a listener to messages indicating when the LLM's speech
   * is played.
   */
  onTokensPlayed = (listener: (ev: TokensPlayedMessage) => void) =>
    this.onMessage(
      (ev) => ev.type === "info" && ev.name === "tokensPlayed" && listener(ev),
    );
}
