 ...
 *
 * // Nested property
 * interpolateString("{{user.profile.firstName}}", { user: { profile: { firstName: "John" } } }); // "John"
 *
 * // Object values are JSON stringified
 * interpolateString("Data: {{data}}", { data: { key: "value" } }); // "Data: {\n  "key": "value"\n}"
 *
 * // Missing values are replaced with empty strings
 * interpolateString("Value: {{missing}}", {}); // "Value: "
 *
 * @throws {Error} Throws an error if there's a problem stringifying an object value
 */

export function interpolateString(template: string, obj: object): string {
