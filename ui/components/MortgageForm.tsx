...
export function MortgageForm({ formName }: { formName: FormNameType }) {
  ...
  const submitD = async () => {
    setIsLoading(true);
    await sleep(500 + Math.floor(1000 * Math.random()));
    setStatus("error");

    if (formName !== "19B-8671-D") return;

    await sleep(10_000 + jitter());
    ...
    addBotMessage(
      "I'm a generalist but I'm part of a larger borg with many AI specialists, powered by Azure Foundry.",
    );
